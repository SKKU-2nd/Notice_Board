...
public class PostRepository
{
    ...
    {
        return _firestore.Collection("posts").Document(); // ID 자동 생성
    }
    ...
    {
        var doc = await _firestore.Collection("posts").Document(postId).GetSnapshotAsync();
        if (!doc.Exists)
            return null;
    ...
    {
        var saveData = new PostSaveData(postDto);
        var docRef = _firestore.Collection("posts").Document(postDto.PostId);
    ...
    {
        var docRef = _firestore.Collection("posts").Document(postId);
